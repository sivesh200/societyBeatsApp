import { ElementRef, Component, OnInit, TemplateRef, Output, EventEmitter } from '@angular/core';
...
@Component({
  selector: 'app-mtbooking',
  templateUrl: './mtbooking.component.html',
  styleUrls: ['./mtbooking.component.css']
})
export class MtbookingComponent implements OnInit {
  ...
  constructor(private modalService: BsModalService, private booking:BookingService,protected global:GlobalService, protected el:ElementRef) {
    this.getCategory();
  ...
  getCategory(){
    try{
      ...
      this.booking.getAllCategory().subscribe(data=>{
  ...
  action(d,r){
    ...
    if(d.id==1){
      ...
      this.currData = r;
      // this.el.nativeElement.querySelector('#showModel').click();
      this.eMode=true;
  ...
  createCategory(){
    ...
    try{
      ...
      if(this.eMode==true){
        ...
        this.booking.updateCategory(param).subscribe(data=>{
          this.global.spin(false);
        ...
        this.booking.createCategory(param).subscribe(data=>{
          this.global.spin(false);
          if(data.success == true){
            this.global.success('Category successfully saved!');
            this.getCategory();
